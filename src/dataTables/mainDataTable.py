...
class MainDataTable(QWidget):
    def __init__(self, grand_father, *args):
        ...
        data, self.total_pages, self.total_patients = self.get_data(doctor_filter=SharedFunctions.get_main_doctor())
        ...
        status_filter = RegularLabel('الترشيح بواسطة ::    حالة المراجعة : ')
    ...
    def setmydata(self, data):
        ...
        self.table.setColumnWidth(1, math.floor(table_width * 0.17))
        self.table.setColumnWidth(2, math.floor(table_width * 0.17))
        self.table.setColumnWidth(3, math.floor(table_width * 0.11))
        self.table.setColumnWidth(4, math.floor(table_width * 0.11))
        ...
        self.table.setVerticalHeaderLabels(
            [" " + str(i + 1 + ((self.current_page - 1) * self.per_page)) + " " for i, row in enumerate(data)])
    ...
    def reset_filters(self, the_exception=None):
        filters = [self.status_filter_select, self.date_filter_select, self.search_input]
    ...
    def get_active_filter(self):
        filters = [self.status_filter_select, self.date_filter_select, self.search_input]
        active_filter = {"doctor_filter": SharedFunctions.get_main_doctor()}
        for the_filter in filters:
            if the_filter == self.status_filter_select:
