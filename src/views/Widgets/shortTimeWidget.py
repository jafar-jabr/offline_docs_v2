...
class ShortTimeWidget(QWidget):
    ...
    def __init__(self, **kwargs):
        ...
        main_layout.setContentsMargins(20, 0, 20, 0) #(left, top, right, bottom)
    ...
    @staticmethod
    def value():
        the_hour = int(ShortTimeWidget.hour_value)
        the_minute = ShortTimeWidget.minute_value
        if ShortTimeWidget.am_pm_value == "م":
            the_hour = 12+int(ShortTimeWidget.hour_value)
        if len(str(the_hour)) == 1:
            the_hour = "0"+str(the_hour)
        if len(ShortTimeWidget.minute_value) == 1:
    ...
    def update_hour(self, hour):
        ShortTimeWidget.hour_value = hour

    def update_minute(self, minute):
        ShortTimeWidget.minute_value = minute

    def update_am_pm(self, am_pm):
        ShortTimeWidget.am_pm_value = am_pm
