...
class AddStaffModal(QDialog):
    def __init__(self, parent):
        ...
        self.setObjectName("add_staff_modal")
        self.line_width = 480
    ...
    def do_add_staff(self, name, phone, job, role, specialization=None):
        ...
        for check, message in [Validator().validate_name(name),
            ...
            if not check:
                ...
                return
        name_exist = Database().check_if_staff_name_exist(name)
        phone_exist = Database().check_if_staff_phone_exist(phone)
        if name_exist:
        ...
        Database().insert_staff(clinic_id, name, phone, job_id, role_id, specialization, created_at)
        MessageBoxes.success_message("تأكيد!", "تم اضافة المنتسب, يستطيع الان تسجيل الدخول")
        self.parent.data_table.update()
        self.accept()
